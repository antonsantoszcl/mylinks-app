...
export function Sidebar() {
  ...
  const handleDeleteDashboard = async (id: string) => {
    ...
    if (!dash) return;
    if (!confirm(`Excluir o dashboard "${dash.title}"? As categorias dentro dele também serão removidas.`)) return;
    await deleteDashboard(id);
    // If we're currently on this dashboard, redirect to default
    if (pathname === `/dashboard/${id}`) {
      const defaultDash = dashboards.find((d) => d.isDefault && d.id !== id) ?? dashboards.find((d) => d.id !== id);
      if (defaultDash) {
